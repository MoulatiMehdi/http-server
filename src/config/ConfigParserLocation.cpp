...
const ConfigParser::locationHandlers ConfigParser::locationEntry[] = {
    ...
    &ConfigParser::handleLocCgiExtension,
    &ConfigParser::handleLocCgiPath
};
...
// cgi_extension <ext>
void ConfigParser::handleLocCgiExtension(LocationConfig& loc) {
    advance();
    if (_tokens[_i].type != TOK_WORD)
        throwError("cgi_extension: expected extension");
    const std::string ext = _tokens[_i].value;
    if (ext.empty() || ext[0] != '.') // validator function
        throwError("cgi_extension: must start with '.'");
    loc.cgi_extension = ext;
    advance();
    expect(TOK_SEMICOLON, "cgi_extension: expected ';' after " + loc.cgi_extension, true);
}

// cgi_path <path>
void ConfigParser::handleLocCgiPath(LocationConfig& loc) {
    advance();
    if (_tokens[_i].type != TOK_WORD)
        throwError("cgi_path: expected path");
    const std::string path = _tokens[_i].value;
    if (!isValidPath(path))
        throwError("cgi_path: invalid path");
    loc.cgi_path = path;
    advance();
    expect(TOK_SEMICOLON, "cgi_path: expected ';' after " + loc.cgi_path, true);
}
